...
class UniSRecDataset(SequentialDataset):
    ...
    def load_plm_embedding(self):
        ...
        # Build ASIN->row index mapping using the SAME order used to create All_Beauty.None
        item_text_path = Path(self.dataset_path) / 'item_text.json'
        with open(item_text_path, 'r') as f:
            _item_text = json.load(f)
        asin2idx = {asin: j for j, asin in enumerate(_item_text.keys())}

        mapped_feat = np.zeros((self.item_num, self.plm_size))
        for i, token in enumerate(self.field2id_token['item_id']):
            if token == '[PAD]': continue
            idx = asin2idx.get(token)
            if idx is None:
                continue
            mapped_feat[i] = loaded_feat[idx]
        return mapped_feat
